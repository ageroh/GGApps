...
def get_ios_app_versions():
	app_pages = (
		"https://itunes.apple.com/gr/artist/travel-applications/id668571732?iPhoneSoftwarePage=1", 
		"https://itunes.apple.com/gr/artist/travel-applications/id668571732?iPhoneSoftwarePage=2"
	)
	ret = []
	ios_app_urls = []
	
	for app_page in app_pages:
		ios_page = htmldom.HtmlDom(app_page).createDom()
		ios_rows = ios_page.find("div[metrics-loc]").first().find("a.artwork-link")
		ios_app_url_fmt = re.compile(r"https://itunes.apple.com/gr/app/.*greekguide\.com.*")
		ios_app_urls = ios_app_urls + [m.group(0) for m in (re.search(ios_app_url_fmt, r.attr('href')) for r in ios_rows) if m]
